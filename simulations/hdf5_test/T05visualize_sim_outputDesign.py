...
triggered = np.array(fin['station_101']['triggered'])
xx = np.array(fin['xx'])
yy = np.array(fin['yy'])
zz = np.array(fin['zz'])
...
for i in range(len(args.inputfilename) - 1):
    ...
    triggered = np.append(triggered, np.array(fin['station_101']['triggered']))
    xx = np.append(xx, np.array(fin['xx']))
    yy = np.append(yy, np.array(fin['yy']))
    zz = np.append(zz, np.array(fin['zz']))
    maxAmp = np.append(maxAmp, np.array(fin['station_101']['maximum_amplitudes']), axis = 0)
    receive_vectors = np.append(receive_vectors, np.array(fin['station_101']['receive_vectors']), axis=0)
    theta = np.append(theta, np.array(fin['zeniths']))
    phi = np.append(phi, np.array(fin['azimuths']))
    polarization = np.append(polarization, np.array(fin['station_101']['polarization']).flatten())
    launch_vectors = np.append(launch_vectors, np.array(fin['station_101']['launch_vectors']), axis=0)
    n_events += fin.attrs['n_events']
vrms = np.ones(len(weights)) * fin.attrs['Vrms']

VRMS = np.ones(7) * fin.attrs['Vrms']
VRMS[6] /= math.sqrt(8.0)
sortedMaxAmp = maxAmp
for i in range(len(sortedMaxAmp)):
    sortedMaxAmp[i] /= VRMS
    sortedMaxAmp[i].sort()
sortedMaxAmp = np.transpose(sortedMaxAmp)
thresholdLeast = np.zeros(len(weights))
for i in range(4):
    thresholdLeast += np.square(sortedMaxAmp[i])
recoLeast = np.array([i > 17.64 for i in thresholdLeast])

maxAmp = np.transpose(maxAmp)
threshold = np.zeros(len(weights))
for i in range(len(maxAmp) - 1):
    threshold += np.square(maxAmp[i] / vrms)
threshold += np.square(maxAmp[len(maxAmp) - 1] / vrms * math.sqrt(8.0))
reco = np.array([i > 900.0 for i in threshold])
...
#Veff = V * density_ice / density_water * 4 * np.pi * float(len(weights)) / n_events # calculating unweighted veff
#print("Veff = {:.6g} km^3 sr".format(Veff / units.km ** 3))

#for multiple triggers only
...
for i in range(len(multiple_triggers)):
    ...
    VeffReco = V * density_ice / density_water * 4 * np.pi * np.sum(weights[np.logical_and(multiple_triggers[i].astype(bool), reco)]) / n_events
    VeffRecoLeast = V * density_ice / density_water * 4 * np.pi * np.sum(weights[np.logical_and(multiple_triggers[i].astype(bool), recoLeast)]) / n_events
    print("Veff{} = {:.6g} km^3 sr".format(i, Veff / units.km ** 3))
    print("VeffReco{} = {:.6g} km^3 sr".format(i, VeffReco / units.km ** 3))
    #print("VeffRecoLeast{} = {:.6g} km^3 sr".format(i, VeffRecoLeast / units.km ** 3))
...
VeffReco = V * density_ice / density_water * 4 * np.pi * np.sum(weights[np.logical_and(np.logical_or(multiple_triggers[0].astype(bool), multiple_triggers[1].astype(bool)), reco)]) / n_events
VeffRecoLeast = V * density_ice / density_water * 4 * np.pi * np.sum(weights[np.logical_and(np.logical_or(multiple_triggers[0].astype(bool), multiple_triggers[1].astype(bool)), recoLeast)]) / n_events
print("VeffTot = {:.6g} km^3 sr".format(Veff / units.km ** 3))
print("VeffTotReco = {:.6g} km^3 sr".format(VeffReco / units.km ** 3))
#print("VeffTotRecoLeast = {:.6g} km^3 sr".format(VeffRecoLeast / units.km ** 3))
print("\n")

#from here start to making plots
...
plt.hist2d(rr / units.m, zz / units.m, bins=[np.arange(0, 10001, 100), np.arange(-4001, 0, 100)], cmap=plt.get_cmap('Blues'), weights=weights/n_events)
cb = plt.colorbar()
...
plt.grid(True)
plt.subplot(2, 1, 2)
plt.hist2d(rr / units.m, zz / units.m, bins=[np.arange(0, 10001, 100), np.arange(-4001, 0, 100)], cmap=plt.get_cmap('Blues'), weights = np.ones(len(rr))/n_events)
cb = plt.colorbar()
...
#plot xy overview
plt.scatter(xx / units.m, yy / units.m, 10)
plt.xlabel("x(m)")
plt.ylabel("y(m)")
plt.axis([-8000, 8000, -8000, 8000])
plt.grid(True)
plt.savefig(os.path.join(plot_folder, 'overview.pdf'), bbox_inches="tight")
plt.clf()
